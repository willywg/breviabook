...
Stand up a fully-tooled, CI-green Python project skeleton for Brevia: build config,
...
`mypy --strict` pass clean and `brevia --help` works.
...
- `brevia/config.py` — pydantic-settings + `.env` loading.
- `brevia/cli.py` — `typer` app with a `condense` command **stub** (parses all flags from
  ROADMAP §8, prints the parsed config; no pipeline yet) and a real `version` command.
- `brevia/llm/base.py` (Protocol), `brevia/llm/factory.py`, `brevia/llm/providers/ollama.py`.
...
- [ ] **SSRF guard:** never forward provider API keys to an arbitrary `--api-endpoint` host
      ...
      `brevia/utils/security.py` even if endpoints land fully in Phase 9.
...
1. `pyproject.toml`: `[project]` (name=brevia, py>=3.11, console script `brevia = "brevia.cli:app"`),
...
3. `brevia/config.py`: `Settings(BaseSettings)` mirroring ROADMAP §9 (`LLM_PROVIDER`,
...
4. `brevia/llm/base.py`: `class LLMProvider(Protocol)` + a shared message type.
   `brevia/llm/factory.py`: `get_provider(name, settings) -> LLMProvider`.
   `brevia/llm/providers/ollama.py`: `OllamaProvider` via litellm.
5. `brevia/utils/security.py`: `validate_safe_path()` (zip-slip, used Phase 1) + helper that
   refuses to attach API keys when the target host isn't the configured provider host (SSRF).
6. `brevia/cli.py`: `typer.Typer()`; `version` command; `condense` command accepting every flag
...
- `LICENSE`, `NOTICE`
- `brevia/__init__.py`, `brevia/cli.py`, `brevia/config.py`
- `brevia/llm/{base,factory}.py`, `brevia/llm/providers/ollama.py`
- `brevia/utils/security.py`
...
- [ ] `uv run brevia --help` and `uv run brevia version` work.
- [ ] `uv run brevia condense book.epub --dry-run --provider ollama --model gemma4:e4b`
...
- [ ] Project installs (`uv sync`) and `brevia` console script runs.
